...
clang++ $includes -g $src -o $build_dir/$target.exe $libs $warnings
...
# GAME                                 #
########################################
src="sandbox/src/game.cpp"
timestamp=$(date +%s)

rm -f $build_dir/game_*
start_time=$(date +%s%N)

clang++ -g $src -shared -o $build_dir/game_$timestamp.dll $warnings
...
# ASSETS & FILES                       #
########################################
cp engine/lib/glfw3.dll $build_dir